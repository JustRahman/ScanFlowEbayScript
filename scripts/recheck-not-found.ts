...
async function main() {
  console.log('=== Recheck NOT FOUND books ===\n');
  if (seller) console.log(`Filtering by seller: ${seller}\n`);

  // Fetch all NOT FOUND books
  ...
  while (true) {
    let query = supabase
      .from('ebay_books')
      .select('*')
      .eq('decision', 'NOT FOUND')
      .range(from, from + pageSize - 1);
    if (seller) query = query.eq('seller', seller);
    const { data, error } = await query;

    if (error) {
      console.error('Error fetching NOT FOUND books:', error.message);
      break;
    }
  ...
  console.log(`Found ${books.length} NOT FOUND books to recheck\n`);
  ...
  for (let i = 0; i < books.length; i += BATCH_SIZE) {
    ...
    for (const book of batch) {
      ...
      if (!product) {
        stillNotFound++;
        console.log(`  [${evaluated}/${books.length}] ${book.isbn} — still no Keepa data`);
        continue;
      }
  ...
  console.log('\n=== Summary ===');
  console.log(`Rechecked: ${evaluated}`);
  console.log(`  BUY: ${buy}`);
